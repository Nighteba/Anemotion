...
/**
 * 
 * Inspire par "Introduction to Programming using Java"
 * ( http://math.hws.edu/eck/cs124/javanotes7/c13/ex2-ans.html ),
 * 
 */
public class PaintPanelBeta extends JPanel 
{
	...
	public BufferedImage copyOSC() 
	{
		...
		Graphics2D elemgraphique = img.createGraphics();
		elemgraphique.drawImage( OSC, 0, 0, OSC.getWidth(), OSC.getHeight(), null);
		elemgraphique.dispose();
		return img;
	}
	 ...
	 */
	public enum Tool { CURVE, LINE, RECT, OVAL, FILLED_RECT, FILLED_OVAL, 
	...
	public void paintComponent(Graphics g) 
	{
		...
		/* Creation off-screen canvas si il n'existe pas deja. */ 
	...
	public JMenuBar getMenuBar() {
		...
		JMenu strokeWidthMenu = new JMenu("Epaisseur");
		JMenu strokeColorMenu = new JMenu("Couleur");
		JMenu fillColorMenu = new JMenu("Remplissage");
		JMenu toolMenu = new JMenu("Outil");
		...
		JMenuItem item;
		item = new JMenuItem("Ouvrir un fichier image...");
		item.setAccelerator(KeyStroke.getKeyStroke("ctrl O"));
		item.addActionListener(listener);
		fileMenu.add(item);
		item = new JMenuItem("Enregistrer en PNG...");
		item.setAccelerator(KeyStroke.getKeyStroke("ctrl S"));
		item.addActionListener(listener);
		fileMenu.add(item);
		item = new JMenuItem("Enregistrer en JPEG...");
		item.addActionListener(listener);
		fileMenu.add(item);
		...
		createRadioGroup( strokeWidthMenu,
				new String[] {
						"Epaisseur = 1/ctrl 1", "Epaisseur = 2/ctrl 2", "Epaisseur = 3/ctrl 3", 
						"Epaisseur = 4/ctrl 4",
						"Epaisseur = 5/ctrl 5", "Epaisseur = 7", "Epaisseur = 10", "Epaisseur = 15",
						"Epaisseur = 20", "Epaisseur = 25"
		}, listener );

		createRadioGroup( strokeColorMenu, 
				new String[] {
						"Tracer en noir/ctrl B", "Tracer en blanc/ctrl W", "Tracer en rouge", 
						"Tracer en vert", "Tracer en bleu", "Tracer en jaune", 
						"Choix de couleur.../ctrl C"
		}, listener);

		createRadioGroup( fillColorMenu, 
				new String[] {
						"Remplir en Black/ctrl shift B", "Remplir en White/ctrl shift W", "Remplir en Red", 
						"Remplir en Green", "Remplir en Blue", "Remplir en Yellow", 
						"Choix de remplissage.../ctrl shift C"
		}, listener);
	...
	private class MenuHandler implements ActionListener {
		public void actionPerformed(ActionEvent evt) {
			...
			if (command.equals("Choix de couleur...")) {
			...
			else if (command.equals("Choix de remplissage...")) {
				Color newColor = JColorChooser.showDialog(PaintPanelBeta.this, 
			...
			else if (command.startsWith("Epaisseur = "))
				lineWidth = Integer.parseInt(command.substring(12));
			else if (command.equals("Tracer en noir"))
				currentStrokeColor = Color.BLACK;
			else if (command.equals("Tracer en blanc"))
				currentStrokeColor = Color.WHITE;
			else if (command.equals("Tracer en rouge"))
				currentStrokeColor = Color.RED;
			else if (command.equals("Tracer en vert"))
				currentStrokeColor = Color.GREEN;
			else if (command.equals("Tracer en bleu"))
				currentStrokeColor = Color.BLUE;
			else if (command.equals("Tracer en jaune"))
				currentStrokeColor = Color.YELLOW;
			else if (command.equals("Remplir en noir"))
				currentFillColor = Color.BLACK;
			else if (command.equals("Remplir en blanc"))
				currentFillColor = Color.WHITE;
			else if (command.equals("Remplir en rouge"))
				currentFillColor = Color.RED;
			else if (command.equals("Remplir en vert"))
				currentFillColor = Color.GREEN;
			else if (command.equals("Remplir en bleu"))
				currentFillColor = Color.BLUE;
			else if (command.equals("Remplir en jaune"))
			...
			else if (command.equals("Ouvrir un fichier image..."))
				doOpenFile();
			else if (command.equals("Enregistrer en PNG..."))
				enregistrerFichier("PNG");
			else if (command.equals("Enregistrer en JPEG..."))
				enregistrerFichier("JPEG");
			else if (command.equals("Quit"))
				System.exit(0);
			else if (command.equals("Retour arriere")) {
	 ...
	 */
	private void enregistrerFichier(String format) {
		if (fileDialog == null)
			fileDialog = new JFileChooser();
	...
	public static void main(String[] args) {
		JFrame fenetre = new JFrame("Canevas");
		PaintPanelBeta content = new PaintPanelBeta();
		fenetre.setContentPane(content);
		fenetre.setJMenuBar(content.getMenuBar());
		fenetre.pack();  
		fenetre.setResizable(true); 
		Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
		fenetre.setLocation( (screenSize.width - fenetre.getWidth())/2,
				(screenSize.height - fenetre.getHeight())/2 );
